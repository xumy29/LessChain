...
contract TBStorage {
    ...
    function addTB(ContractTB memory tb, bytes[] memory sigs, bytes[] memory vrfs, 
        ...
        for (uint32 i = 0; i < sigs.length; i++) {
            ...
            if (!addrRecorded[signers[i]] || addr2Shard[signers[i]] != tb.shardID) {
                emit LogMessage("address not recorded", tb.shardID, tb.height, signers[i]);
                continue;
            }
            // step 1. check the vrf result is generated by signer
            if (!verifyVrfResult(seed, vrfs[i], signers[i])) {
                emit LogMessage("vrf verification not passed", tb.shardID, tb.height, signers[i]);
                continue;
            }
            // step2. check that the vrfValue is qualified for being a signer
            if (!vrfIsQualified(vrfs[i])) {
                emit LogMessage("vrf not qualified", tb.shardID, tb.height, signers[i]);
                continue;
            }
            ...
            if (verifySignature(tb, sigs[i], signers[i])) {
                ...
                emit LogMessage("signature verification not passed", tb.shardID, tb.height, signers[i]);
            }
        }

        if (validSigCnt < minSigCnt) {
            emit LogMessage("insufficient valid signatures", tb.shardID, tb.height, address(0));
        } else {
